...
function getHomeTeam()
{
    ...
    foreach ($html->find(".eqc text") as $team) {
        if ($team == "MONTPELLIER H&#201;RAULT SC") {
            $team = str_replace("&#201;", "É", $team);
        }
        $getHomeTeam1[] = $team;
    }
    $i = 0;
...
function colorTeam($teamName)
{
    $jsonTeamColor = json_decode(file_get_contents("../../assets/data/teamColor.json"), true);
    foreach($jsonTeamColor as $teamNameColor) {
        if($teamNameColor['team'] == $teamName){
            return $teamNameColor['hex'];
        }
    }
}
    ...
    function buildJson()
{
    ...
    for ($i = 0; $i < 381; $i++) {
        ...
        $json .= '"colorHomeTeam":"' . colorTeam($homeTeam[$i]) . '",';
        $json .= '"colorAwayTeam":"' .  colorTeam($awayTeam[$i]) . '",';
        $json .= "},";
        $c++;
...
{
   file_put_contents('../../assets/data/ligue1_API.json', buildJson());
}
